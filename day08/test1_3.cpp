...
int main(){
    ...
    cout << "Estimate Correction Method" << endl;
    cout << "---------------" << endl;
    cout << "h\t\ty(3)\t\treal_y(3)\terror" <<endl;
    cout << "---------------" << endl;
    
    for (double h = 1.0; h >= 1.0/128; h /= 2.0){
        cout << "1/" << (int)(1/h) << "\t\t" << EC_Euler(a, b, t0, y0, h) << "\t" << 3.0*exp(-3/2.0)-2+3.0<< "\t" << 3.0*exp(-3/2.0)-2+3.0 - EC_Euler(a, b, t0, y0, h) << endl;
    }
    cout << "---------------" << endl;
