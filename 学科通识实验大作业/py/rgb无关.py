...
min_array_bgr_trans = cv2.merge((min_r_trans,min_g_trans,min_b_trans))

min_array_brg_trans = cv2.merge((min_g_trans,min_r_trans,min_b_trans))

min_array_grb_trans = cv2.merge((min_b_trans,min_r_trans,min_g_trans))

min_array_gbr_trans = cv2.merge((min_r_trans,min_b_trans,min_g_trans))

min_array_rbg_trans = cv2.merge((min_g_trans,min_b_trans,min_r_trans))
...
#cv2.imwrite('min_array_rgb.jpg', min_array_rgb)
#cv2.imwrite('min_array_rgb_trans.jpg', min_array_rgb_trans)

color = ['b','g','r']

images = [img_clr[:,:,[2,1,0]], 0, 
          min_array_rgb[:,:,[2,1,0]], 0, 
          min_array_rgb_trans[:,:,[2,1,0]], 0,
          min_array_bgr_trans[:,:,[2,1,0]], 0, 
          min_array_brg_trans[:,:,[2,1,0]], 0, 
          min_array_grb_trans[:,:,[2,1,0]], 0, 
          min_array_gbr_trans[:,:,[2,1,0]], 0, 
          min_array_rbg_trans[:,:,[2,1,0]], 0]

# 绘制图像
titles = ['Original', 'Histogram', 
          'Min RGB', 'Histogram',
          'Min RGB Trans', 'Histogram'
          'Min BGR Trans', 'Histogram',
          'Min BRG Trans', 'Histogram',
          'Min GRB Trans', 'Histogram',
          'Min GBR Trans', 'Histogram',
          'Min RBG Trans', 'Histogram'
          ]
