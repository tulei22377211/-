...
int main(int argc, char *argv[])
{
    ...
    for(int i = 0 ; i < isetRow;i++)
    {
        for(int j= 0;j <jsetColumn;j++)
        {
            ...
            std::cout <<std::setw(13)<<  con_f(U[i][j])  << ",     ";
        }
        std::cout << std::endl;
    }
    ...
    for(int i = 0 ; i < isetRow;i++)
    {
        for(int j= 0;j <jsetColumn;j++)
        {
            ...
            std::cout <<std::setw(13)<<  con_f(L[i][j])  << ",     ";
        }
        std::cout << std::endl;
    }
    ...
    for(int k = 1; k<isetRow;k++){
        for(int j = k; j<isetRow;j++){
            usumstep = 0.0;
            for(int m =0;m<k;m++){
                usumstep = con_f(usumstep + con_f(L[k][m]*U[m][j]));
            }
            U[k][j] = con_f(A[k][j] - usumstep);
        }
        for(int i = k+1; i<isetRow;i++){
            usumstep = 0.0;
            for(int m =0;m<k;m++){
                usumstep = con_f(usumstep + con_f(L[i][m]*U[m][k]));
            }
            L[i][k] = con_f((con_f(A[i][k] - usumstep)) / U[k][k]);
        }
    }
    ...
    for(int k = 1; k<isetRow;k++){
        ysumstep = 0.0;
        for(int i =0; i<k; i++){
            ysumstep = con_f(ysumstep + con_f(L[k][i]* y[i]));
        }
        y[k] =con_f( b[k] - ysumstep);
    }
    ...
    for(int i = 0 ; i < isetRow;i++)
    {
        ...
        std::cout <<std::setw(13)<<  con_f(y[i]) << std::endl;
    }

     std::cout << std::endl;

    x[isetRow-1] = con_f(y[isetRow-1]/U[isetRow-1][isetRow-1]);

    float xsumstep = 0.0;
    for(int i = isetRow - 2; i>-1;i--){
        xsumstep = 0.0;
        for(int j =i+1; j<isetRow; j++){
            xsumstep = con_f(xsumstep + con_f(U[i][j]* x[j]));
        }
        x[i] = con_f((con_f(y[i] - xsumstep))/U[i][i]);
    }
    ...
    for(int i = 0 ; i < isetRow;i++)
    {
        ...
        std::cout <<std::setw(13)<<  con_f(x[i]) << std::endl;
    }
